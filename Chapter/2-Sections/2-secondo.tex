...
\subsection{Deduzione dell'equazione}
...
Inoltre può essere fattorizzata nel prodotto tra una funzione che dipende solo da $r$, detta \textit{parte radiale} $R(r)$ e una funzione che dipende solo dagli angoli $\theta$ e $\varphi$ detta \textit{parte angolare} $\rchi(\theta, \varphi)$:
...
\begin{itemize}
  ...
  \item 3s, 3p e 3d degeneri ($\rm 1s + 3p + 5d=9$ livelli).
\end{itemize}
...
La differenza nella capacità di penetrazione determina la separazione in energia degli orbitali s, p, d ed f a parità di numero quantico principale negli atomi polielettronici.
