...
\vspace{0.2cm}La pressione, cioè la forza esercitata sulle pareti, ha come unità di misura l'\textit{atmosfera} ($\rm atm$). Inoltre $\rm 1 \; atm=760 \; mmHg=1 \; Torr$. Nel S.I. l'unità di misura è il \textit{Pascal} ($\rm Pa$), per cui

$$P=\rm N/m^2=\big[ (kg \cdot m)/s^2 \big]/m^2=(kg \cdot m)/(s^2 \cdot m^2)$$
$$\implies P=\rm kg/(s^2 \cdot m)$$

Spesso nelle bombole l'unità di misura è il \textit{bar}. 1 bar equivale a $10^5 \; \rm Pa$.

La temperatura si misura in gradi centigradi ($^{\circ} \rm C$) o in gradi Kelvin (K). Per passare da Celsius a Kelvin aggiungiamo 273.15:
...
Consideriamo l'O$_2$. Notiamo che a $25 \; ^{\circ} \rm C$ la porzione di molecole che ha velocità elevata è ristretta, mentre a $1000 \; ^{\circ} \rm C$ tale porzione è molto maggiore.
...
$$P=\text{cost} \implies V_t=V_0(1 + \alpha t), \; \text{con} \; \alpha=\frac{1}{273}$$
$$\implies V_t=V_0 \left( \frac{273 + t}{273} \right), \; T=273 + t$$
$$\implies V_T = \frac{V_0 T}{273}, \; \frac{V_T}{T}=\frac{V_0}{273}=\text{cost}$$
...
$$V=\text{cost} \implies P_t=P_0(1 + \alpha t), \; \text{con} \; \alpha=\frac{1}{273}$$
$$\implies P_t=P_0 \left( \frac{273 + t}{273} \right), \; T=273 + t$$
$$\implies P_T = \frac{P_0 T}{273}, \; \frac{P_T}{T}=\frac{P_0}{273}=\rm cost$$
...
$$\begin{array}{ll}
    P_1=175 \; \text{Torr} & P_2=125 \; \text{Torr}\\[1ex]
    V_1=0.527 \; \text{L} & V_2=3.141 \; \text{L}
\end{array}$$

\end{minipage}

\vspace{0.4cm}A temperatura costante vale $PV=\rm cost$.

Il volume finale vale $V_{tot}=V_1 + V_2 =3.668 \; \rm L$

Le pressioni parziali valgono

\vspace{0.2cm}$P_1V_1=P_x \cdot 3.668 \rightarrow 175 \cdot 0.527 = P_x \cdot 3.668 \rightarrow P_x = 25.14 \; \rm Torr$

\vspace{0.2cm}$P_2V_2=P_y \cdot 3.668 \rightarrow 125 \cdot 3.141 = P_y \cdot 3.668 \rightarrow P_y = 107.4 \; \rm Torr$

\vspace{0.2cm}La pressione totale vale $P_{tot}=P_x + P_y=25.14 + 107.04=132.18 \; \rm Torr$. Essa sarà la pressione esercitata dal volume $V_{tot}$, che è calcolata in funzione delle pressioni parziali grazie alla legge di Dalton.
...
\vspace{0.2cm}"\textit{Volumi uguali di gas diversi, nelle stesse condizioni di temperatura e pressione, devono contenere lo stesso numero di molecole. In particolare a $0 \; ^{\circ} \rm C$ e $1 \; \rm atm$ una mole di qualunque gas occupa sempre un volume di} $22.414$ \textit{litri, che è detto volume molare}".

\vspace{0.2cm}Si definiscono \textbf{condizioni normali} $0 \; ^{\circ} \rm C$ e 1 atm.

\vspace{0.2cm}Si definiscono \textbf{condizioni standard} $25 \; ^{\circ} \rm C$ e 1 atm.
...
Ricordiamo che una mole contiene $6.023 \cdot 10^{23}$ particelle, tale numero si può determinare così:
...
Supponiamo di avere dell'acqua e di riscaldarla fino a $200 \; ^{\circ} \rm C$. Essa, raggiunta la temperatura di $100 \; ^{\circ} \rm C$, rimarrà a tale temperatura finché tutta l'acqua non passa allo stato di vapore. Dopodiché crescerà la temperatura del vapore.
