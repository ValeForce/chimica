...
$$\ce{Na+ + \mathcircled{\text{OH^- + H^+}} + Cl- ->Na+ + Cl- + H2O}$$%dio porco non riesco a sistemarlo, vaffanculo chemformula pogjaoihgdjfia
...
In queste reazioni bisogna stare attenti al nome dell'acido che stiamo usando, ad esempio con lo zolfo abbiamo sia acido solforoso \ce{H_2SO_3} che deriva dall'anidride solforosa \ce{SO_2} dove lo zolfo ha valenza 4, sia acido solforico \ce{H_2SO_4} che deriva dall'anidride solforica \ce{SO_3} dove lo zolfo ha valenza 6. Se in una reazione usiamo il primo, il sale che si forma si chiama solfito, se si usa il secondo il sale si chiama solfato:
\begin{center}
...
Consideriamo ad esempio l'acido solforico \ce{H_2SO_4}. Avendo due protoni, la sua dissociazione avverrà in due step:
...
Per semplicità finora abbiamo immaginato (e continueremo a fare così in futuro) che l'\ce{H_2SO_4} si dissoci in un unico step:
...
Imponendo che l'\ce{NaOH} sia stechiometricamente uguale all'\ce{HSO_4} si riesce a neutralizzare/salificare solo uno dei due protoni dell'acido. Ciò avviene perché quest'ultimo si dissocia in due step diversi, per cui gli ioni \ce{OH-} della base neutralizzeranno solo la metà degli ioni \ce{H+} dell'acido, mentre l'altra metà resterà invariata.
...
$$\ch{2 Al(OH)_3 + 3 H_2SO_4 -> Al_2(SO_4)_3 + 6 H_2O}$$
...
Nel primo i 3 idrogeni sono legati ognuno ad un atomo di ossigeno (cioè abbiamo tre gruppi OH), mentre nel secondo un idrogeno è legato direttamente al fosforo. Questo legame \ch{H-P} è covalente, per cui questo idrogeno non sarà salificabile (cioè non può essere sostituito da un metallo). Infatti per essere tali, gli idrogeni devono aver formato un legame polare. Quindi è inutile far reagire ad esempio una molecola di \ce{H_3PO_3} con tre di \ce{NaOH}, perché ci sono solo due gruppi OH che possono dar luogo a salificazione:
$$\ch{H_3PO_3 + 3 NaOH -> Na_3PO_3 + 3 H_2O}$$
...
Le dissociazioni dell'acido fosforoso pertanto sono
...
In questo caso al posto dell'idrossido abbiamo un metallo. Il processo è lo stesso, solo che stavolta non si forma \ce{H_2O} in quanto abbiamo solo gli ioni \ce{H+} dell'acido, bensì viene rilasciato idrogeno in forma gassosa (indicato dalla freccia $\uparrow$ nella reazione)